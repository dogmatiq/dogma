...
A **command** is a request to perform some specific application-defined
action, such as "add 10 widgets to Anna's shopping cart".

- An **event** represents a fact within the application's history, such as
  "10 widgets added to Anna's shopping cart".

Messages are the fundamental unit of communication in a Dogma application. Every
interaction — internal or external — begins and ends with a message. They are
the only way to trigger behaviour, and the only output your application
produces.
